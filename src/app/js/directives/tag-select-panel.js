...
class TagSelectPanel extends SelectPanel {
    ...
    updateTagsState() {
        ...
        if (this.rawData.tagManager.starredTags.length > 0) {
            this.rawData.tagManager.starredTags.forEach((tag) => {
                const existTagItem = this.rawData.tagItemsMap[tag];
    ...
    updateItemList() {
        ...
        // 初始化各個群組
        let selectedGroup = { id: this.GROUP_ID.SELECTED, items: [], isCollapsed: false, name: i18n.__('selectTagPanel.label.selected') };
        let starredGroup = { id: this.GROUP_ID.STARRED, items: [], isCollapsed: false, name: i18n.__('selectTagPanel.label.starred') };
        let historyGroup = { id: this.GROUP_ID.HISTORY, items: [], isCollapsed: false, name: i18n.__('selectTagPanel.label.recent') };
        let suggestGroup = { id: this.GROUP_ID.SUGGEST, items: [], isCollapsed: false, name: i18n.__('selectTagPanel.label.recommend') };
        let normalGroup = { id: this.GROUP_ID.NORMAL, items: [], isCollapsed: false, name: i18n.__('selectTagPanel.label.others') };
        let tagGroupsMaps = {};
