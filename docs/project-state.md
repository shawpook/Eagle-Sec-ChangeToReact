...
| **M1** 优先修功能闭环 | 🔄 **进行中** | F15 ✅ / F13-preview ✅ / F10 🔄 / F06 🔄(前后端) / F08·F09 待实现 / F04 未派 |
...
| **W10** | task_1ec4ff82fdf1 | ctx_0fb8e0d33d86 | **M1-4 F06 后端图像变换端点** | 🔄 运行中（心跳：闭环测试已绿，跑全量回归） |
...
2. W10（F06 后端变换端点）→ 收结果、核验（重点：非对称图旋转后**读回像素**、失败时不写盘）、合并
3. W11（F06 前端失败语义）→ 收结果、核验、合并

### 立即可派（依赖已满足）
4. **F08/F09 动作供给修复**（依据 W7 报告）
   - 注意：与 W8 的 F10 都在 `externalSupplyRegistrar.ts` / `driverApi.ts` 上有重叠 →
     **必须等 W8 合并后再派**，否则两个编码 Worker 会冲突
   - 范围：补字体改名/星标/激活状态/拖拽的真实供给；消除内部不必要的字符串派发
     （`Toolbar.tsx` 的 `scope[fn]` → 直 import 具名 action）；缺必需动作时断言或显式错误
5. **F06 第三批：前端接上真实能力**（依赖 W10 + W11 合并）
   - JPEG 走渲染层 EXIF 无损路径（解 `moduleRegistry.ts:106-107` 的桩）
   - 非 JPEG 走后端端点；按格式分流须有**唯一判定点**
6. **F04 预览窗 boot 移出前置内联脚本**（`src/app/preview-window.html:20-47`）
   - 需真实浏览器 + Electron 控制台验证；与 M3 的 `bundleGlobals` 文本执行问题相邻
7. **M2 实现**（F07/F01/F05）——依据 W9 报告，核心是：
