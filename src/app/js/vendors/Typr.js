...
Typr["parse"] = function(buff)
{
	...
	if(tag=="ttcf") {
		...
		for(var i=0; i<numF; i++) {
			...
			// Note: 不需要解析全部字体信息
			return [readFont(data, i, foff,tmap)];
...
Typr["T"].name = {
	parseTab : function(data, offset, length)
	{
		...
		for(var i=0; i<count; i++)
		{
			...
			console.log(`name: ${str}, languageID: ${languageID}`);
		...
		*/
		...
		for(var p in obj) {
			var ob = obj[p];
			if (ob["_lang"] === 2052 ) { support["cn"] = ob["fullName"]; if (!finalObj) finalObj = ob;  }
			else if (ob["_lang"] === 1028 ) { support["tw"] = ob["fullName"]; if (!finalObj) finalObj = ob;  }
			else if (ob["_lang"] === 1041 ) { support["jp"] = ob["fullName"]; if (!finalObj) finalObj = ob;  }
			else if (ob["_lang"] === 1042 ) { support["kr"] = ob["fullName"]; if (!finalObj) finalObj = ob;  }
			else { support["en"] = ob["fullName"]; }
		}

		var convertResult = function (result) {
			result.supportCN = !!support["cn"];
			result.supportTW = !!support["tw"];
			result.supportJP = !!support["jp"];
			result.supportKR = !!support["kr"];
			result.supportEN = !!support["en"];
			return result;
		}

		if (finalObj) {
			return convertResult(finalObj);
		}

		// for(var p in obj) if(obj[p][psn]!=null && obj[p]["_lang"]==0x0409) return convertResult(obj[p]);		// United States
		// for(var p in obj) if(obj[p][psn]!=null && obj[p]["_lang"]==0x0000) return convertResult(obj[p]);		// Universal
		// for(var p in obj) if(obj[p][psn]!=null && obj[p]["_lang"]==0x0c0c) return convertResult(obj[p]);		// Canada
