...
try {
  ...
  page.on && page.on('Runtime.exceptionThrown', (e) => {
    ...
    console.log('PAGE_EXC: ' + String((d.text || '') + ' ' + ((d.exception && (d.exception.description || d.exception.value)) || '')).slice(0, 500));
  });
  page.on && page.on('Runtime.consoleAPICalled', (e) => {
    const txt = String((e.text ?? '') + ' ' + (e.args || []).map(a => a.value ?? a.description ?? '').join(' ')).slice(0, 300);
    if (/error|Error|EXC|not defined|Cannot/i.test(txt)) console.log('PAGE_LOG: ' + txt);
  });
  ...
  { const pre = await waitExpr(`JSON.stringify({containers: document.querySelectorAll('.swal2-container').length, visible: [...document.querySelectorAll('.swal2-container')].map(c => getComputedStyle(c).display)})`);
    console.log('DBG7D1A-PRE:', pre.result.value); }
  // 决定性实验：观察 confirm 前后 modal className 的每一次变化（摘除后回挂 vs 从未摘除）
  await page.send('Runtime.evaluate', { expression: `(() => {
    window.__mvMut = [];
    const el = document.querySelector('#eagle-move-folder-host .move-folder-modal');
    if (!el) return false;
    window.__mvObs = new MutationObserver((muts) => {
      for (const m of muts) window.__mvMut.push({ t: Date.now(), cls: (m.target.className || '').slice(0, 80) });
    });
    window.__mvObs.observe(el, { attributes: true, attributeFilter: ['class'] });
    window.__mvRej = null; window.__mvErr = null;
    window.addEventListener('unhandledrejection', (e) => { window.__mvRej = String((e.reason && (e.reason.stack || e.reason)) || e.reason).slice(0, 500); });
    window.addEventListener('error', (e) => { window.__mvErr = String((e.error && (e.error.stack || e.error)) || e.message).slice(0, 500); });
    const body = window.$bodyScope;
    const origMove = body.moveFoldersAsSibling;
    window.__mvCalls = [];
    body.moveFoldersAsSibling = function (...a) {
      window.__mvCalls.push({ n: a.length, isBottom: a[2] === true, sel: (a[0]||[]).map && a[0].map(x => x.name).join('|') });
      return origMove.apply(this, a);
    };
    return true;
  })()`, returnByValue: true });
  await page.send('Runtime.evaluate', {
  ...
  await delay(500);
  { const post = await waitExpr(`JSON.stringify({containers: document.querySelectorAll('.swal2-container').length, swalOpen: !!document.querySelector('.swal2-container'), movedParent: window.$bodyScope.folderMappings[window.__mvFav] && window.$bodyScope.folderMappings[window.__mvFav].parent})`);
    console.log('DBG7D1A-POST:', post.result.value); }
  { const mut = await waitExpr(`JSON.stringify(window.__mvMut)`);
    console.log('DBG7D1A-MUT@POST:', mut.result.value); }
  { const trap = await waitExpr(`JSON.stringify({rej: window.__mvRej, err: window.__mvErr, calls: window.__mvCalls})`);
    console.log('DBG7D1A-TRAP:', trap.result.value); }
  { const fiberPost = `(() => {
      const el = document.querySelector('#eagle-move-folder-host .move-folder-modal');
      if (!el) return 'no-el';
      ...
      while (f && hops < 40) {
        if (f.memoizedState) {
          ...
          while (h && i < 10) {
            ...
            if (v && typeof v === 'object' && 'open' in v) return 'view.open@POST=' + v.open;
            h = h.next; i++;
          }
          return 'no-view-hook';
        }
    ...
    const fp = await waitExpr(fiberPost);
    console.log('DBG7D1A-FIBER@POST:', fp.result.value); }
  await delay(300);
  { const dbg = await waitExpr(`JSON.stringify({
      mapLen: Object.keys(window.$bodyScope.folderMappings || {}).length,
      targetMapped: !!window.$bodyScope.folderMappings['${targetFolderId}'],
      targetParent: window.$bodyScope.folderMappings['${targetFolderId}'] && window.$bodyScope.folderMappings['${targetFolderId}'].parent,
      favMapped: !!window.$bodyScope.folderMappings['${favFolderId}']
    })`);
    console.log('DBG7D1A-MAP:', dbg.result.value); }
  await assertExpr(
  ...
  await delay(600);
  { const dbg = await waitExpr(`JSON.stringify({
      cls: document.querySelector('#eagle-move-folder-host .move-folder-modal') && document.querySelector('#eagle-move-folder-host .move-folder-modal').className,
      swalOpen: !!document.querySelector('.swal2-container'),
      overlay: !!document.querySelector('#eagle-move-folder-host .move-folder-modal-overlay')
    })`);
    console.log('DBG7D1A:', dbg.result.value);
    const mut2 = await waitExpr(`JSON.stringify(window.__mvMut)`);
    console.log('DBG7D1A-MUT@FINAL:', mut2.result.value); }
  { const fiberDump = `(() => {
      const el = document.querySelector('#eagle-move-folder-host .move-folder-modal');
        ...
        if (f.memoizedState) {
          ...
          while (h && i < 10) {
            ...
            if (v && typeof v === 'object') {
              if ('open' in v) desc = 'view{open=' + v.open + ',selectedFolders=' + (v.selectedFolders ? v.selectedFolders.length : '?') + '}';
              else if (v.current !== undefined || v.baseState !== undefined) desc = '[ref/queue]';
              else desc = '[obj]';
            } else desc = String(v);
            hooks.push(desc);
            h = h.next; i++;
          }
          return JSON.stringify({hops, hooks, elIsConnected: el.isConnected});
        }
        f = f.return; hops++;
      }
      return JSON.stringify({err: 'no-hooks'});
    })()`;
    const fd = await waitExpr(fiberDump);
    console.log('DBG7D1A-FIBER:', fd.result.value); }
  await assertExpr(
    'mv-closed',
    `!document.querySelector('#eagle-move-folder-host .move-folder-modal').classList.contains('open')`
  );
