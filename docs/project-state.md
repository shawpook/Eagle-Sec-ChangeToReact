...
> 最后更新：2026-09-16（**M1–M7 全部完成并集成**；M8 进行中：第一轮 L3 已跑、3 个沙箱回归已修、补登记 3 个孤儿测试（98 项）；W49/W50 在跑）
> 当前套件 **102 项** + ARTIFACT **6 项**；最近提交 `58fa2ca5`。
> **最终验收（四段）已跑**：static / build / artifact / regression **四段全 PASS**（`FRONTEND_ACCEPTANCE_ALL_GREEN`，
> 套件 102 项 ALL GREEN + 套件外 7 项全 PASS）；**`backend` 段 FAILED**——真因见 D40，W56 在修。
...
| **W56** | task_3196a685a00e | ctx_7100c075f03a | **M8-7 后端 `fs.cpSync(recursive)` 改手动递归拷贝（依据 D40）** | 🔄 进行中（worktree `m8-cpsync`） |
...
| **D40** | **`fs.cpSync(recursive)` 在本机打死进程——用户裁决「改那三处」** | M8 最终验收的 `backend` 段失败，Coordinator 定位到真因：`fs.cpSync(src, dst, {recursive:true})` 在本机一碰到**含嵌套子目录**的树就把进程打死（退出码 `3221226505` = `0xC0000409` STATUS_STACK_BUFFER_OVERRUN，**无异常栈**）。取证：纯 Node 里同样崩（exit 127）；逐条目二分显示「单个文件 OK、`images/` 与 `backup/` 这类含子目录的目录必崩」；**合成一棵深度 4 的最小树也必崩** ⇒ 不是夹具特有。**后果两层**：① `POST /api/library/migrate`、`exportLibrary`、备份恢复三个端点在本机不可用；② `npm test` 是 `&&` 链，在第 **36/47** 步（`library-migration`）断掉，**后面 11 个测试在本机从未执行过**（`security` / `roadmap-panels` / `library-transaction-faults` / `library-crash-recovery` / `library-concurrency` / `library-consistency-scan` / `library-backup-restore` / `library-repair-plan` / `source-mode-api` / `source-mode-thumbnails` / `source-mode-watch`）。**用户裁决改那三处**（另加 `tests/roadmap-panels.mjs:139` 那处，它在链的第 37 步）。依据是仓内**已用过两次**的同一手法：`backend/src/server.js:2070-2083` 的 `copyInfoDirRecursive()`（注释明写「宿主 fs.cpSync(recursive) 缺陷绕行」）与 `frontend/vite.preview.config.mjs` 的 `copyTree()`。**W56 / M8-7** 执行，并要求它如实报告那 11 个测试第一次跑起来后暴露了什么 |
