...
| P1 通道桥 | 🟡 **P1-a（`dfd95ab8`）+ P1-b（本批）已落地**，P1-c…e 未落地。P1-a：`core/channelBridge.getIpcBus()` 收口主窗 17 文件 24 处取总线表达式。P1-b：`preload.cjs` 新增通用 `ipc` 桥；`getIpcBus()` 升级为**接缝 facade**——事件面（on/emit/…）前转 shims 总线，发送面按通道二分（纯原生直通 6 频道 + `smoke:*` 走 preload，其余仍走 shims 路由表），`invoke` 必须先走 shims；`eagleGlobals.ipcRenderer()` 改返 facade（不得直接返 `desktop.ipc`，否则绕过接缝）。新增 `tests/react-ipc-bridge-routing.mjs` 断言单路由不变量（已入套件）。**P1-c…e 未落地**：把 desktopApi 路由族 + 21 个合成事件 + `desktopApi.onIpc` 扇出迁入 facade 后，shims 才只剩浏览器 mock（P4），再删 shims（P5）。逐信道地图见 `docs/e5-5-ipc-inventory-2026-09-14.md` |
