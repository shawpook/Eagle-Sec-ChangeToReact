...
| P1 通道桥 | ✅ **P1-a（`dfd95ab8`）+ P1-b（`df043f08`）+ P1-c-1（`b9be8f5d`）+ P1-c-2（`919d7ef5`）已落地**。P1-c-2：新增 `core/ipcWriteState.ts`（itemUpdateQueue/enqueueWrite/whenWriteQueueIdle、导入在飞计数 trackLocalImport/importCounters、mergeCachedItems、emitImportedItems、导入缩略图回填（快照前排空写队列）、调色板分析、customThumbnailItemIds、emitEvent——语义逐字对齐 shims），安装为 `window.__eagleIpcWriteState`；`channelBridge.routeDesktop` 接管 **images-change/image-change**（入队 clone 快照、返回体逐条 image.changed、isDeleted→duplicates.merge、成败各一的 item:operation-result，经共享实例发布）；shims 删约 490 行写路径实现，消费方（capture 轮询/导入/缩略图/调色板/startLifecycle）全部经 `writeState()` 取**同一实例**（无 React 窗口惰性自建等价实例，窗内唯一）。验证：**先验收后迁移**——`tests/electron-write-path-closed-loop.mjs`（main→backend 边界替身 + 真实编辑入口：单路由 8=8、入队快照、顺序、延迟、失败隔离、多选、star）基线 3/3 → 迁移后 3/3；`tests/item-persistence-closed-loop.mjs`（真实后端+临时库+编辑/重启读回六项）基线 3/3 → 迁移后 3/3。P1-b 引入的 m1/cz3 总线取用回归已独立修复（`b3019028`）。**P1-c-3…e 未落地**（导入/剪贴板族 + 合成事件全量迁入 + 回程扇出；shims 仍为浏览器 mock 传输，见 §6.1 后续）。逐信道地图见 `docs/e5-5-ipc-inventory-2026-09-14.md` |
| P2 详情门控 | ✅ 已落地（选项①超恢复接线）。先实测出门控在 React UI 路径下不生效（包装 scope 面/store 侧 `enterDetailMode`，UI 各入口直调 import 的 `machineryEnterDetailMode`，非同一函数对象——探针 `tests/probe-detail-gate-reachability.mjs`）。后新增 `core/detailDeliveryGate.ts`，由 `machineryEnterDetailMode`（选区守卫前）/`machineryLeaveDetailMode` 内部直接挂钩；`shims.js` 删 177 行门控 + 25ms 包装体，仅保留**驱动面**的 document-workspace 分支（`--smoke-document-viewer` 依赖）。探针复查 pathA/pathB 双入口均生效。详见 `docs/e5-5-shims-retirement-plan.md` §0.1/§0.2。**遗留：属可见行为变化，需实机走查** |
| P3 source-mode UI | ✅ **P3-b 已落地（本批）**：功能补建 + 迁移（shims 的 source-mode 块约 500 行删除，改由 React/store 实现）。新增 `store/sourceModeState.ts` + `core/sourceMode.ts`（打开/关闭/目录导航/展开/重扫/移除/添加/退出还原，`generation` 守卫防过期响应）+ `components/sidebar/SourceModeSidebar.tsx`（树/管理两个页签，真实 onClick；CSS 迁移自 shims）；`Sidebar.tsx` 的「切换文件夹」按钮改来源模式切换入口（原 `ng-click` 兼容钩子退役）；shims 暴露 `__eagleEmitMockLifecycle` 作为网格喂料种子入口。**关键**：`getVirtualLibrary` 的 items 混排所有来源根——`filteredItems()` 沿 folders 树收集目标目录 folderId 后代集合后**真正按 (root, relativePath) 过滤**（含子目录）再喂网格。重扫/移除语义对照用户决策逐条实现（失败保留上次数据 + 重试提示；当前根移除选侧栏第一个剩余；无来源留空状态；退出还原原目录/筛选/排序/仍有效选区——种子异步落定后 450/1200ms 双次快照重放）。收口验收 `tests/source-mode-browse-closed-loop.mjs`（3 来源根含嵌套目录 + 真实点击 CDP）：根=全部含子目录、子目录=该目录及嵌套、跨目录选区清除、展开只动树、重扫增/删、目录消失回根、失败提示（后端测试旋钮 `EAGLE_SOURCE_RESCAN_FAIL`）、移除非当前/当前/最后来源、退出还原网格+keyword——3/3 稳定全绿；现有 `source-mode-ui-closed-loop` 复跑绿。P2 遗留（详情门控可见行为）与 P3 已清，见 §0.1/§0.3 更新 |
| P4 浏览器 mock 隔离 | ⬜ 未落地 |
...
| `main-ui-workflow`（E7 已定位并修复，剩余低频变体） | 该测试是**唯一**覆盖 Electron 侧 item 持久化（`images-change`→`updateMany`）的端到端测试。2026-09-14 曾持续失败且失败步逐次不同，三次对照（stash P2 / `bb538f9e` pre-P1-a 树 / 当前树）证明**基线亦失败**。E7 定位批以渲染层 send/sendTo 载荷探针 + live 对象属性 setter 抓栈完成根因定位：① 回归 A（`919d7ef5`）：shims 剪贴板分支 `writeState().writeState()` TypeError；② 根因 C（既有）：`mergeCachedItems` 的 `Object.assign` 写到 live 对象本体（cache 条目与 live 同引用），导入期调色板分析携带导入初值合并把在飞编辑打回（栈：mergeCachedItems→Object.assign→annotation setter）——修复为 cache 存副本 + 调色板分析限定字段域 + 接缝回执改差分；③ 根因 D（既有）：capture 轮询跨 tick 双发 file-uploaded + 消费者无幂等 → 重复 id；④ 回归 B：driver restore 直发 shims 总线（P1-c-2 删分支后落黑洞）→ 改走接缝。验证：连续 6 轮 **4 绿**（改前 0 绿），含重启校验完整闭环通过；**剩余** `no-target-id` identity 变体（m1 族，6 轮 1 次）留档待续。提交：`5a3a98be`（写路径修复）+ `dad7b24e`（驱动加固） |
