...
const SmartFolder = require('./model/smart-folder.js');
...
global.require = (args) => {
	...
	if (args === 'fs' || args === 'original-fs') return eagleFS;

	const pluginModulePath = `${eagle?.plugin?.path}/node_modules/${args}`;
	if (eagle?.plugin?.path && require('fs').existsSync(pluginModulePath)) {
...
pluginIPC.addCallback('plugin-create', async(plugin) => {
	...
	try {
		...
		if (dependencies.indexOf("ai-sdk") > -1) {
			const pluginPath = `${eagle.app.userDataPath}/Plugins/ai-sdk`;
			const aiModulePath = `${pluginPath}/ai-sdk.cjs`;
			const ai = require(aiModulePath).default({
				configPath: pluginPath + "/config.bin"
			});
...
pluginIPC.addCallback('theme-changed', onThemeChanged);
...
global.eagle = {
	...
	app: {
		...
		show: async () => {
			return await ipcRenderer.r2r(global?.parentID, 'app.show');
		}
	},
	os: {
		tmpdir: os.tmpdir,
