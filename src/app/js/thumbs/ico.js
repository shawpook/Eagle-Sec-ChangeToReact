...
module.exports = async ({ src, dest, item }) => {
    return new Promise(async (resolve, reject) => {
        try {
            let result = await nativeThumb.async({ src: src, dest: dest, size: 480, item: item });

            if (!fs.existsSync(dest)) {
                return reject(new Error(`ICO thumbnail generate fail.`));
            }

			let thumbSize;
			try {
				thumbSize = await imageSize.async(src);
			}
			catch (err) {}
