// b1-9bq probe8: boot-failure diagnosis (console capture before scope)
import fs from 'node:fs'; import os from 'node:os'; import path from 'node:path';
import { fileURLToPath } from 'node:url';
const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'eagle-bq-p8-'));
...
try {
  ...
  const logs = [];
  const collect = (e) => { logs.push(String((e.text ?? '') + ' ' + (e.args || []).map(a => a.value ?? a.description ?? '').join(' ')).slice(0, 300)); };
  page.on && page.on('Runtime.consoleAPICalled', collect);
  page.on && page.on('Runtime.exceptionThrown', (e) => logs.push('EXC: ' + JSON.stringify(e.exceptionDetails).slice(0, 400)));
  ...
  logs.filter((l) => /error|Error|EXC|fail/i.test(l)).slice(0, 10).forEach((l) => console.log('LOG:', l));
} finally {
