...
try {
  ...
  await page.send('Runtime.evaluate', {
    expression: `(function () {
      ...
      window.__b1_9ah.frame = f;
      f.onload = function () {
        setTimeout(function () {
          try {
            const d = f.contentDocument;
            const ins = d.getElementById('gif-player-ins');
            window.__b1_9ah.glue = d.body.className.indexOf('render-normal') > -1
              && !!(ins && ins.getAttribute('src') || '').length
              && (ins.getAttribute('src') || '').indexOf('file:///') > -1;
          } catch (err) { window.__b1_9ah.glue = false; }
        }, 300);
      };
      document.body.appendChild(f);
      const poll = setInterval(function () {
        try {
          const d = f.contentDocument;
          window.__b1_9ah.bodyCls = d ? d.body.className : null;
          var loaded = window.__b1_9ah.bodyCls && window.__b1_9ah.bodyCls.indexOf('loaded') > -1;
          const s = window.$bodyScope;
          window.__b1_9ah.isGifReady = s ? s.isGifReady : null;
          window.__b1_9ah.frames = s && s.gifViewer && s.gifViewer.frames ? s.gifViewer.frames.length : null;
          if (loaded && s && s.isGifReady === true && Array.isArray(s.gifViewer.frames) && s.gifViewer.frames.length > 0) {
        ...
        } catch (err) { window.__b1_9ah.pollErr = String(err && err.message); }
      }, 250);
      setTimeout(function () {
        try {
          const d = f.contentDocument;
          const ins = d.getElementById('gif-player-ins');
          window.__b1_9ah.insSrc = ins && ins.getAttribute('src');
          const xhr = new f.contentWindow.XMLHttpRequest();
          xhr.open('GET', window.__b1_9ah.insSrc, false);
          try { xhr.send(); window.__b1_9ah.xhr = 'status:' + xhr.status; } catch (e2) { window.__b1_9ah.xhr = 'throw:' + e2.message; }
          window.__b1_9ah.hasSuperGif = typeof d.defaultView.SuperGif;
          window.__b1_9ah.xhrProbeDone = true;
        } catch (err) { window.__b1_9ah.xhr = 'outer:' + err.message; window.__b1_9ah.xhrProbeDone = true; }
      }, 8000);
      setTimeout(function () {
        window.__b1_9ah.done = true;
        clearInterval(poll);
  ...
  try {
    const r = await page.send('Runtime.evaluate', { expression: 'JSON.stringify(window.__b1_9ah)', returnByValue: true });
    console.log('GIF_DIAG ' + r.result.value);
  } catch (err) { console.log('GIF_DIAG_ERR ' + err.message); }
