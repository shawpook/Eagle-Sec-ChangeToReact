...
try {
  ...
  await page.send('Runtime.evaluate', {
    expression: `window.$bodyScope.$apply(() => { window.$bodyScope.selected = [window.$bodyScope.allData[0]]; window.$bodyScope.$broadcast('UPDATE_INSPECTOR'); })`,
    returnByValue: true,
  });
  await assertExpr(
  ...
  // ── 多选 → selected-count ──
  await page.send('Runtime.evaluate', {
    expression: `window.$bodyScope.$apply(() => { window.$bodyScope.selected = [...window.$bodyScope.allData]; window.$bodyScope.$broadcast('UPDATE_INSPECTOR'); })`,
    returnByValue: true,
  });
  await assertExpr(
