...
export function takeoverLibraryDomain(): void {
  ...
  ipc.on('library.changed', function (_event: any, newLibrary: any) {
    ...
    eagle.utils.tree.walk(newFolders, 'children', function (folder: any) {
      ...
      if (useItemState.getState().folderMappings[folder.id]) {
        folder.images = useItemState.getState().folderMappings[folder.id].images;
    ...
    eagle.utils.tree.walk(newLibrary.smartFolders, 'children', function (smartFolder: any) {
      ...
      if (useItemState.getState().smartFolderMappings[smartFolder.id]) {
    ...
    writeSmartFolders(newLibrary.smartFolders || []);
  ...
  ipc.on('app-status-library-loaded', async function (_e: any, params: any) {
    ...
    if (w.RecentFileManager && w.RecentFileManager.init) w.RecentFileManager.init(useMiscRawState.getState().libraryName);
    if (useMiscRawState.getState().TagManager && useMiscRawState.getState().TagManager.init) useMiscRawState.getState().TagManager.init(params.rootDir);
    ...
    if (w.eagle && w.eagle.utils && w.eagle.utils.tree) {
      ...
      writeSmartFolders(params.smartFolders || []);
    ...
    machineryCalculateImageBinding({}, function () {
      ...
      if (useBodyState.getState().viewMode == "all") {
        ...
        const lastFolder = useItemState.getState().folderMappings[lastFolderId as any];
        const lastSmartFolder = useItemState.getState().smartFolderMappings[lastFolderId as any];

        if (lastFolder && !lastFolder?.password) {
          openFolder(lastFolder);
          machineryChangeSidebarIndex(lastFolder);
          ...
          machineryOpenAll(undefined, function () {
            setTimeout(function () {
              const DAY_7 = 604800000;
    ...
    ipc.on('background-state', function (_evt: any, state: any) {
      ...
      requestAnimationFrame(() => {
        ...
        if (useMiscRawState.getState().currentProcessCount > 0) {
    ...
    if (typeof (window as any).ACCESS?.checkALCs === 'function') {
      if (!ACCESS.checkALCs(useMiscRawState.getState().libraryPath)) {
        ...
        if (useToastState.getState().libraryPathPermissionError !== true) {
          writeLibraryPathPermissionError(true);
        }
      }
      else {
...
export function machineryGetChildFoldersMap(folder: any): any {
  const w = window as any;
  var childs: any = {};
...
export function machineryGetFolderParentChilder(folder: any): any {
  if (folder.parent && useItemState.getState().folderMappings[folder.parent]) {
    return useItemState.getState().folderMappings[folder.parent].children;
  }
  else {
    return useFolderState.getState().folders;
  }
}

export function machineryImportLinks(): void {
  const w = window as any;
  ...
  w.swal({
    ...
    const currentFolderId = useFolderState.getState().currentFolder?.id;
...
export function machineryOpenArtstation(): void {
  importArtstationChannel.emit();
}

export function machineryOpenHuaban(): void {
  const w = window as any;
...
export function machineryGetChildFoldersMaps(folders: any): any {
  const w = window as any;
  var childs: any = {};
...
export function machineryGetFolderImages(folder: any, includeSubFolder: any): any[] {
  const w = window as any;
  var images: any[] = [];
...
export function machineryGetRecentFolders(length?: any): any[] {
  ...
  recentMoveFolders = recentMoveFolders.filter(function (folderId: any) {
    return !!useItemState.getState().folderMappings[folderId];
  });
...
export function machinerySaveFolder(): void {
  const w = window as any;
  ...
  var folders: any[] = [];
  w.cloneTree(folders, useFolderState.getState().folders);

  w.eagle.utils.tree.walk(folders, 'children', function (folder: any, parent: any, depth: any) {
    folder.name = nfc(folder.name);
  });

  const smartFolders = useFolderState.getState().smartFolders.map(function (smartFolder: any) {
    var clone: any = {
      id: smartFolder.id,
  ...
  const groups = useMiscRawState.getState().TagManager.groups.map(function (group: any) {
    var g: any = {
      id: group.id,
      name: nfc(group.name),
      tags: group.tags
  ...
  const quickAccess = useMiscRawState.getState().quickAccess.map(function (item: any) {
    return {
      type: item.type,
      id: item.id
    };
...
export function machineryAddToRecentFile(item: any): void {
  const w = window as any;
  // 記錄在最近使用
  setTimeout(function () {
    if (useSelectionState.getState().current !== item) {
...
export function machineryChangeSidebarIndex(node: any): void {
...
export function machineryGetAllChildFolder(fd: any): any[] {
  const w = window as any;
  let folders: any[] = [];
...
export function machineryGetFolderList(): any[] {
  const w = window as any;
  let list: any[] = [];
...
export function machineryGetQuickAccessList(): any[] {
  var list: any[] = [];
  useMiscRawState.getState().quickAccess.forEach(function (item: any) {
    item.size = 27;
...
export function machineryGetSmartFolderList(): any[] {
  const w = window as any;
  let list: any[] = [];
  let isFiltering = !!useMiscRawState.getState().folderKeyword;
  let guidelinesMap: any = {};
  ...
  w.eagle.utils.tree.walk(useFolderState.getState().smartFolders, 'children', function (smartFolder: any, parent: any, depth: any) {
    ...
    // 列表版本 SmartFolders
    useMiscRawState.getState().smartFolderList.push(smartFolder);
    syncSidebarFromScope();
...
export function machineryOpenNextFolder(): void {
  ...
  var idx = folders.indexOf(useFolderState.getState().currentFolder);
  nextFolder = folders[idx + 1];

  if (nextFolder) {
    openFolder(nextFolder);
    machineryChangeSidebarIndex(nextFolder);
  }
}

export function machineryOpenNextSmartFolder(): void {
  var nextSmartFolder;
  var listItems = useMiscRawState.getState().sidebarList;
  var smartFolders = listItems.filter(function (item: any) {
    return item.vstype === 'smartFolder' || item.vstype === 'smartFolderGroup';
  });
  var idx = smartFolders.indexOf(useFolderState.getState().currentSmartFolder);
  nextSmartFolder = smartFolders[idx + 1];
  if (nextSmartFolder) {
    openSmartFolder(nextSmartFolder);
    machineryChangeSidebarIndex(nextSmartFolder);
  }
  else {
    var folders = listItems.filter(function (item: any) {
      return item.vstype === 'folder';
    ...
    if (folders.length > 0) {
      ...
      machineryChangeSidebarIndex(folders[0]);
    }
  }
}

/* openParentFolder（bundle 38384-38388 逐字；openFolder 经 scope 解析） */
export function machineryOpenParentFolder(): void {
  if (useFolderState.getState().currentFolder && useFolderState.getState().currentFolder.parent) {
    openFolder(useItemState.getState().folderMappings[useFolderState.getState().currentFolder.parent]);
  }
}

export function machineryOpenPrevFolder(): void {
  const w = window as any;
  ...
  if (prevFolder) {
    ...
    machineryChangeSidebarIndex(prevFolder);
  }
  else {
    ...
    if (smartFolders.length > 0 && smartFolders[smartFolders.length - 1]) {
      ...
      machineryChangeSidebarIndex(smartFolders[smartFolders.length - 1]);
    }
...
export function machineryOpenPrevSmartFolder(): void {
  const w = window as any;
  var prevSmartFolder;
  var listItems = useMiscRawState.getState().sidebarList;
  var smartFolders = listItems.filter(function (item: any) {
...
export function machineryOpenRecent(ignoreHistory?: any): void {
  ...
  if (useBodyState.getState().viewMode === 'recent' && useItemState.getState().allData.length > 0 && w.eagle.filter.filterRules.color.value == undefined) {
  ...
  openRecentTimeout = $timeout(function () {
    ...
    useLayoutState.getState().imageSize.height = w.localStorage.getItem("eagle.list.thumbSize.recent") || 150;
    syncToolbarFromScope();
    syncBodyFromScope();
    syncDetailFromScope();
    syncInspectorFromScope();
...
export function machineryOpenTrash(ignoreHistory?: any): void {
  ...
  openTrashTimeout = $timeout(function () {
    ...
    useLayoutState.getState().imageSize.height = w.localStorage.getItem("eagle.list.thumbSize.trash") || 150;
    syncToolbarFromScope();
    syncBodyFromScope();
    syncDetailFromScope();
    syncInspectorFromScope();
...
export function machineryOpenUnfiled(ignoreHistory?: any): void {
  ...
  if (useBodyState.getState().viewMode === 'unfiled' && useItemState.getState().allData.length > 0 && w.eagle.filter.filterRules.color.value == undefined) {
    if (useBodyState.getState().isDetailMode) {
      machineryLeaveDetailMode();
    }
    return;
  ...
  openUnfiledTimeout = $timeout(function () {
    ...
    useLayoutState.getState().imageSize.height = w.localStorage.getItem("eagle.list.thumbSize.unfiled") || 150;
    syncToolbarFromScope();
    syncBodyFromScope();
    syncDetailFromScope();
    syncInspectorFromScope();
...
export function machineryQuickOpenFolder(folder: any, t: any): void {
  const w = window as any;
  const $timeout = getTimeout();
  var target = t || useSelectionState.getState().selected[0];
  if (folder) {
  ...
  if (target) {
    setTimeout(function () {
      for (var i = useItemState.getState().allData.length - 1; i >= 0; i--) {
        var image = useItemState.getState().allData[i];
        if (target === image) {
...
export function machineryRemoveFolder(folder: any, params: any = {}): void {
  const w = window as any;
...
export function machineryRemoveFolderContents(params: any): void {
  const w = window as any;
  const $timeout = getTimeout();
  ...
  useSelectionState.getState().selected.forEach(function (image: any) {
    ...
    useFolderState.getState().currentFolder.imagesMappings[image.id] = false;
    if (useFolderState.getState().currentFolder.parent) {
      if (useItemState.getState().folderMappings[useFolderState.getState().currentFolder.parent].imagesMappings) {
        useItemState.getState().folderMappings[useFolderState.getState().currentFolder.parent].imagesMappings[image.id] = false;
      }
    }

    // 遍歷所有子資料夾，移除子資料夾也出現這張圖的索引
    if (useFolderState.getState().currentFolder.children) {
      useFolderState.getState().currentFolder.children.forEach(function (child: any) {
    ...
    if (!isForceToTrash && image.folders && image.folders.length > 1) {
      var idx = image.folders.indexOf(useFolderState.getState().currentFolder.id);
      if (idx > -1) {
  ...
  $timeout(function () {
    machineryForceFitImageSize(useSelectionState.getState().current, undefined);
    machineryZoom();
  }, 100);
  w.ScrollbarSaver.saveScrollPosition();
...
export function machineryRemoveFolderInner(folder: any, { isDeleteImages, ignoreSelectNext, ignoreRestore }: any = {}): void {
  const w = window as any;
  ...
  if (!ignoreRestore) {
    w.cloneTree(originalFolders, useFolderState.getState().folders, true);
  }

  // 找到包含 folder 的 list
  var parent = useItemState.getState().folderMappings[folder.parent];
  var children = (parent) ? parent.children : useFolderState.getState().folders;
  ...
  if (folder.children && useMiscRawState.getState().quickAccess.length > 0) {
    w.eagle.utils.tree.walk(folder.children, 'children', function (child: any, parent: any) {
  ...
  if (!ignoreRestore) {
    ...
    (useMiscRawState.getState().notify || useMiscRawState.getState().notify).call(undefined, {
      ...
      writeFolders(originalFolders);

      w.eagle.utils.tree.walk(useFolderState.getState().folders, 'children', function (folder2: any, parent: any) {
        if (!folder2.children) { folder2.children = []; }
        if (folder2 && parent) { folder2.parent = parent.id; }
        useItemState.getState().folderMappings[folder2.id] = folder2;
      });
      ...
      machineryCalculateImageBinding({ ignoreSort: true }, function () {
        openFolder(useItemState.getState().folderMappings[folder.id]);
...
export function machineryRemoveSmartFolder(smartFolder: any, _p: any = {}): void {
  const w = window as any;
  setTimeout(function () {
    ...
    w.swal({
      ...
      machineryRemoveSmartFolderInner(smartFolder, {});
    });
  }, 100);
}

export function machineryRemoveSmartFolderInner(smartFolder: any, { ignoreSelectNext, ignoreRestore }: any = {}): void {
  const w = window as any;
  const $timeout = getTimeout();
  ...
  delete useItemState.getState().smartFolderMappings[smartFolder.id];
  ...
  if (!ignoreRestore) {
    (useMiscRawState.getState().notify || useMiscRawState.getState().notify).call(undefined, {
      ...
      if (smartFolder.parent && useItemState.getState().smartFolderMappings[smartFolder.parent]) {
        ...
        writeSmartFolders(origin);
      }
      useItemState.getState().smartFolderMappings[smartFolder.id] = smartFolder;
      w.eagle.utils.tree.walk(useFolderState.getState().smartFolders, 'children', function (sf: any, parent: any, depth: any) {
        useItemState.getState().smartFolderMappings[sf.id] = sf;
      });
      machineryUpdateSidebarList();
...
export function machinerySetLastFolder(folderId: any): void {
  const w = window as any;
  if (!setLastFolderDebounced) {
    setLastFolderDebounced = debounce(function (fid: any) {
      if (!fid) {
        ...
        localStorage.setItem(`eagle.lastFolder.${useMiscRawState.getState().rootDir}`, fid);
      }
    }, 500);
  }
...
export function machineryShowTutorial(): void {
  const w = window as any;
  if (useItemState.getState().all.length === 0 && useMiscRawState.getState().libraryHistory.length === 1 && useFolderState.getState().folders.length === 0) {
    if (w.localStorage["eagle.show.tutorial"] !== "true") {
      const theme = getFilter()('themePath')(useBodyState.getState().theme);

      w.swal({
        html: `
                            <div class="alert">
                                <div class="alert-image">
                                    <img width="384" height="216" src="assets/images/${theme}/illustrations/illustration-tutorial-${useBodyState.getState().platform}.png" style="width: calc(100% + 32px);margin-bottom: 12px;aspect-ratio: 768/432;margin-left: -16px;margin-right: -16px;margin-top: -16px;">
                                </div>
...
export function machineryUpdateSidebarList(): void {
  ...
  updateSidebarListTimeout = $timeout(function () {
    ...
    var folders = machineryGetFolderList();
    var smartFolders = machineryGetSmartFolderList();
    var quickAccess = machineryGetQuickAccessList();
