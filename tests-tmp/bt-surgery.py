# -*- coding: utf-8 -*-
"""b1-9bt span 手术：字体操作族 8 + tag 漏网 4 fns（334 行）→ services/fontTagService.ts。"""
import io
import os
import json
...
NAMES = ['deactivateFont', 'activateFont', 'renameFontsWithFullName', 'activateFonts',
         'deactivateFonts', 'changeFontDefaultLang', 'isFontActivate', 'getFontPath',
         'filterWithTag', 'renameTagGroupKeyup', 'renameTagGroupBlur', 'onTagSidebarResize']
...
os.makedirs('tests-tmp/bt-extracts', exist_ok=True)
io.open('tests-tmp/bt-extracts/spans.json', 'w', encoding='utf-8').write(
...
io.open('tests-tmp/bt-extracts/all-fns.ts', 'w', encoding='utf-8', newline='').write(all_src + '\n')
...
print('手术完成，行数', len(lines), '；12 fns 合计', sum(e - c + 1 for c, e in spans.values()), '行')
