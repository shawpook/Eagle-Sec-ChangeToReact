...
try {
  ...
  await delay(1200);
  { const dbg = await waitExpr(`JSON.stringify({vm: window.$bodyScope.viewMode, cf: window.$bodyScope.currentFolder && window.$bodyScope.currentFolder.name, kw: window.$bodyScope.keyword, tags: (window.$bodyScope.filterWithTags||'').slice(0,3)})`);
    console.log('DBG7C2:', dbg.result.value); }
  await assertExpr(
