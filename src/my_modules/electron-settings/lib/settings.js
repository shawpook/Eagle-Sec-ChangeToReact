...
let overwriteSettingsDir;
let overwriteSettingsName;
let remote;
// 表示在 main
if (electron.app) {
...
let mainProcess = (remote && remote.process) || process;

if (mainProcess.argv && mainProcess.argv.length > 0) {
	mainProcess.argv.forEach((arg) => {
		if (arg.indexOf("--settings-path=") > -1) {
			let settingsPath = arg.split("--settings-path=")[1];
			if (fs.existsSync(settingsPath)) {
				overwriteSettingsDir = path.dirname(settingsPath);
				overwriteSettingsName = path.basename(settingsPath);
			}
		}
	});
}
 ...
 * @type {string}
 */
const USER_DATA_PATH = overwriteSettingsDir || app.getPath('userData');
 ...
 * @type {string}
 */
const SETTINGS_FILE_NAME = overwriteSettingsName || 'Settings';
...
class Settings extends EventEmitter {
  ...
  getSync(keyPath) {
    ...
    // NOTE: Hotfix for eagle 4.0 blue theme
    if (value?.theme?.css === "dark-blue") {
      value.theme.css = "blue";
    }
    return value;
  }

  /**
   * Get preferences with automatic migration and processing.
   * This method specifically handles the 'preferences' key with additional logic.
   *
   * @returns {Object} The processed preferences object
   */
  getPreferences() {
    debug('called getPreferences()');
    
    let preferences = this.getSync('preferences');
    ...
    // Perform automatic migrations and processing here
    preferences = this._migratePreferences(preferences);
    
    return preferences;
  }

  /**
   * Migrate preferences data to handle version compatibility and new features.
   *
   * @param {Object} preferences - The raw preferences object
   * @returns {Object} The migrated preferences object
   * @private
   */
  _migratePreferences(preferences) {
    // Create a copy to avoid mutating the original
    const migrated = JSON.parse(JSON.stringify(preferences));
    
    // Ensure shortcuts structure exists
    if (!migrated.shortcuts) {
      migrated.shortcuts = {};
    }
    if (!migrated.shortcuts.keybinds) {
      migrated.shortcuts.keybinds = {};
    }
    
    // Migration map from old keys to new keys
    const keyMigrationMap = {
      'show.eagle': 'global.show.eagle',
      'show.search': 'global.show.search',
      'capture.area': 'global.capture.area',
      'capture.window': 'global.capture.window',
      'capture.full': 'global.capture.full',
      'create.folder': 'file.create.folder',
      'create.smartFolder': 'file.create.smartfolder',
      'import.artstation': 'file.import.artstation',
      'switch.library': 'library.switch',
      'toggle.filter': 'find.filter.toggle',
      'quicksearch': 'find.quicksearch',
      'add.to': 'find.add.to',
      'zoom.in': 'view.zoom.in',
      'zoom.out': 'view.zoom.out',
      'zoom.actual': 'view.zoom.actual',
      'zoom.fit': 'view.zoom.fit',
      'open.link': 'view.file.openlink',
      'view.refresh': 'view.toggle.slideshow'
    };
    
    // Perform key migrations
    const keybinds = migrated.shortcuts.keybinds;
    for (const [oldKey, newKey] of Object.entries(keyMigrationMap)) {
      if (keybinds[oldKey]) {
        // 只有在新 key 不存在時才遷移
        if (!keybinds[newKey]) {
          keybinds[newKey] = keybinds[oldKey];
        }
        // 始終刪除舊 key 以避免重複
