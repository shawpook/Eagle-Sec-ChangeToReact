# -*- coding: utf-8 -*-
"""b1-9br span 手术：文件夹核心操作族 7 fns（618 行）→ services/folderCoreService.ts。"""
import io
import os
import json
...
NAMES = ['createFolder', 'newFolder', 'newFolderWidthSelection', 'addImagesToFolder',
         'moveFoldersAsSibling', 'moveFoldersToFolder', 'emptyRestore']
...
os.makedirs('tests-tmp/br-extracts', exist_ok=True)
io.open('tests-tmp/br-extracts/spans.json', 'w', encoding='utf-8').write(
...
io.open('tests-tmp/br-extracts/all-fns.ts', 'w', encoding='utf-8', newline='').write(all_src + '\n')
...
print('手术完成，行数', len(lines), '；7 fns 合计', sum(e - c + 1 for c, e in spans.values()), '行')
