...
try {
  ...
  await page.send('Runtime.evaluate', {
    expression: `(function () {
      ...
      f.contentWindow && f.addEventListener('error', function (ev) { window.__b1_9ai.err = String(ev.message || ev); });
      fetch('/src/app/text-editor/text-editor.html?id=x&theme=dark&language=zh_CN').then(function (r) {
        return r.text().then(function (t) {
          window.__b1_9ai.fetch = r.status;
          window.__b1_9ai.hasRoot = t.indexOf('text-editor-root') > -1;
          window.__b1_9ai.hasEntry = t.indexOf('react/viewers/text-editor') > -1;
          window.__b1_9ai.hasOld = t.indexOf('TextEditorController') > -1;
          window.__b1_9ai.tail = t.slice(-260);
        });
      }).catch(function (e) { window.__b1_9ai.fetch = 'ERR:' + e.message; });
      const poll = setInterval(function () {
        try {
          const d = f.contentDocument;
          const el = d && d.getElementById('content');
          const rootEl = d && d.getElementById('text-editor-root');
          window.__b1_9ai.rootLen = rootEl ? (rootEl.childNodes || []).length : -1;
          window.__b1_9ai.rootHtml = rootEl ? String(rootEl.innerHTML).slice(0, 120) : null;
          window.__b1_9ai.bodyCls = d ? d.body.className : '';
          const text = el ? (el.textContent || '') : '';
          window.__b1_9ai.textLen = text.length;
  ...
  try {
    const r = await page.send('Runtime.evaluate', { expression: 'JSON.stringify(window.__b1_9ai)', returnByValue: true });
    console.log('TXT_DIAG ' + r.result.value);
  } catch (err) { console.log('TXT_DIAG_ERR ' + err.message); }
