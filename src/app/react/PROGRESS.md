...
>
> **【C-2 ✅ 完成：41 个频道全部迁到 eagleBus（2026-09-10）】**
>
> 五批完成，共 41 个频道 / 约 160 处。终态：`bus.ts` 43 个 `defineChannel`（41 迁移 +
> 2 原有 CONTEXTMENU），34 个文件引用；全树 `$broadcast`/`$on` **只剩 1 处注释**。
>
> - **第一批**：MOVE/RESIZE-CROP-TOOL、SET-FOLDER-PASSWORD、OPEN_RENAME（31 处）
> - **第二批**：REFRESH_VIDEO_COMMENTS、ADD_TO_LIBRARY、OPEN_DUPLICATE_SCAN_PANEL、
>   OPEN_DUPLICATE（19 处）
> - **第三批**：WEBP_CONVERT_START、UPDATE_INSPECTOR、NEW.SMART.FOLDER、
>   INSPECTOR.TAG.SELECT.PANEL.OPEN、SAVE_FOLDER、OPEN_QUICK_SEARCH_MODAL、OPEN_URL_IN_PANEL（18 处）
> - **第四批**：20 个面板/导入/插件类频道（42 处）
> - **第五批（收官）**：REBIND_REFRESH、CALCULATE_IMAGE_BINDING、UPDATE_SELECTION
>   （原与 B6 契约观测点、EXCLUDE spy 名单耦合，本批解耦）+ REFRESH_PLUGIN_CENTER、
>   OPEN_NOTIFICATION、CLOSE-TAGS-POPUP + 补迁被二分回退丢失的 OPEN_DUPLICATE_SCAN_PANEL
>
> **三个必记的坑**（都已固化进工具/复查）：
...
>    被当成 event。含解构形态 `(e, { smartFolder, parent })`、`async (event, categoryId)`、
>    单参占位 `(_e)` 与**具名 handler**（`channel.on(onStart)`）。
> 3. **迁移器的替换正则**：`[\w$]*` 含 `$`，会把 `$root` 当标识符整体匹配 ——
>    `s.$root.$broadcast(` 只被吃掉 `$root.`，前缀 `s.` 残留成 `s.xxxChannel.emit()`
>    （运行期 TypeError）。必须把 `.$root.$broadcast(` 与 `.$broadcast(` 分成两条规则；
>    清扫工具 `tests-tmp/bz-c2-fixprefix.py`（本批共清 47 处，含前几批同类隐患）。
>
> **测试侧契约同步**（合计 70+ 处）：`$root.$broadcast`/`scope.$broadcast` 驱动改
> `window.__eagleBus.emit`；`scope.__bus['CH']` 断言改 `window.__eagleBus.listenerCount('CH')`
> （bus.ts 为此新增该方法）；`$on` 监听改 `bus.on`。
>
> **重要发现**：`REFRESH_PLUGIN_CENTER` / `OPEN_NOTIFICATION` / `CLOSE-TAGS-POPUP` 三个
> 曾被当作「死频道」，实际对端在 `src/app/js/`（**未被 index.html 引入**的原版 Angular 目录），
> React 侧接收端是活的、且 7c/7d5b 契约依赖 —— 已一并迁移，不可删除。
>
> **三个必记的坑**（都已固化进工具/复查）：
> 1. **bus import 路径按目录深度算**（`components/xx/` 是 `../../global/bus`）——写死路径
>    会导致 vite 加载失败、**启动加载链整条断**（与 C-0 同症状）。通用迁移器
...
> **【D-1 收官现状（2026-09-11）：挂载面 180/247 = 73%；函数体归位未开始】**
>
> **已完成六批**（提交 86d786f / abf1985 / af0ba09 / 8faf804 / 0b80fc5 / 2ba2c7f）：
> 挂载 247 → **67**；dataMachinery 11838 → **11673 行**。每批均过 probe + 定向测试。
>
> **剩余 67 个分四类，各有明确阻塞（都已实测撞过）**：
> 1. **契约锚定 ~19**：CONTRACT 名单 16 + **按别名检查的隐藏契约**（m1-A8-relayout-machinery /
>    m1-A9-smart-filter / stage5 detail-* 共 10 断言）。须先改测试（如 m1-D 的
>    spy \ 改为效果断言）。
> 2. **字符串动态调用 ~26**：\ / \ / 映射表。
>    须先改造 call 机制（支持传函数引用而非字符串）。
> 3. **裸引用/谓词传递 ~9**：如 \。批次 4 试改
>    contentFilter + calculateFilterCounts → LOAD_BROKEN（allData=0），已回退。
> 4. **单例 debounce**：\ 右侧会执行（创建 debounce），
>    直调化须先提取单例 getter，否则每次新建、防抖失效。
>
> **函数体归位（11673 行 / 247 导出 → 各 domain/service，文件消失）尚未开始** ——
> 这是 D-1 的另一半，需按域拆多批（每批一个函数簇 + 依赖搬运），是 D 阶段最大的单块工作。
>
> **改写器**：\（4 道检查：字符串动态调用 / 裸引用与存在性检查 /
> 目标必须是真实导出 / 引号内与注释行）。
>
...
**D-2j / D-2g：全局 `$(...)` 清零 + jQuery vendor 退役（2026-09-11，同会话续批）**

> **重要更正（前置）**：哨兵 `jQuery` 指标（`/\bjQuery\(/g`）只统计 `jQuery(...)` 工厂调用，
> **不等于** jQuery 全部消费面；且 `(^|[^a-zA-Z0-9_$.])\$\(` 这类正则会漏掉
> **属性访问形态** `(window as any).$(` / `w.$(`。真实清零须按「裸 `$(` + `.$(` + `$.` 静态」
> 三面普查。
...
| 批次 | 范围 | 结果 |
|---|---|---|
| D-2j-1 | `core/itemDomain.ts`(18) + `core/miscDomain.ts`(16) | 原生 `domQuery` |
| D-2j-2 | services 层 + 组件残余：folderCore/lock/bitmapViewer/imageOps/sidebar/batchOps/libraryDomain/miscMenu/media/viewOps/upload/filterDomain/selection/fontTag/contextMenu + panels/shell/Inspector/inspectorActions/BodyBindings | `.data()` 通道路由 domQuery WeakMap |
| D-2j-3/4 | `core/hoverPreview.ts`(66) + `core/smoothZoomEngine.ts`(47) | 新增自研集合层 `utils/domLite.ts` |
| D-2j-5 | `preview-window/controller.ts`(94) + `Sidebar.tsx`；`$.ajax`×3→fetch、`$.isNumeric`→utils/lang、`$.ui`/`$.fn.hoverIntent` 残留守卫 | 全清 |
| D-2g | `git rm` jquery-1.8.0/long-click/bez；6 处 HTML 引用摘除；`global.js`/`lazy-load-manager.js`/model-viewer/pdf-viewer 壳原生化 | `vendorScriptTags 1→0` |

**新增工具**：`src/app/react/utils/domLite.ts` —— 遗留移植体（hoverPreview/smoothZoomEngine/
preview-window controller，共 4900 行 jQuery 集合式代码）的自研 DOM 集合层，原生实现 jQuery
子集（find/each/append/prepend/appendTo/wrap/clone/css/attr/data/on-bind-off-unbind 命名空间
与委托（含 mouseenter/mouseleave→mouseover/mouseout 过滤）/animate(duration=0)/fadeIn-out/
offset/position/ready/click），布尔属性按 jQuery `boolHook`（false 删属性、真值写 `name="name"`），
`.data()` 与 `domQuery` 共享同一 WeakMap（degree/flip/show-at-zoom 跨文件读写一致）。
与 `utils/domQuery`（元素级）、`components/interactions/*`（交互层）同属项目既定
「原生重写 / 换自研交互层」路线。

**门禁**（无 jQuery vendor 实跑）：
- sentinel：`SENTINEL_OK`，`jQuery 0`、`vendorScriptTags 0`；基线更新为 `b1-9bz-D-2g`
  （`scopeApply 201→200`、`getBodyScope 795→794` 单调下降一并落盘）。
- `bz-export-check.py` 无问题；`probe-b5-load.mjs` LOAD_OK allData=1；tsc 619（<788 基线）。
- 定向套件全绿（摘 vendor 后复跑）：stage-smoke / stage1m1 / stage5 / stage6 / stage9a2。

**D-2 遗留（非 jQuery，不阻塞 jQuery/vendor 清零）**：
1. `frontend/public/shims.js`（3996 行 Angular/bundle shim）退役 —— **被 D-1 Track A 阻塞**：
   `dataMachinery.ts:11316` 明载 enterDetailMode/leaveDetailMode 挂载「shims.js 以 25ms 轮询
   消费，D-2 退役前不可删」；挂载面（原 67 个 machinery 箭头/调用式）未清零前摘 shims 会
   使这些入口静默失效。须待 D-1 A-1~A-5 完成，再以 preload.cjs 等价供给替换。
2. `src/app/collect-window/js/vendors/jquery-1.8.0.min.js` + jquery-ui —— 采集窗是独立 HTML/API
   层（自带 `js/lib/api/*`、`swal-dialog.js`/`env.js` 仍消费 jQuery），与主窗共享路径
   `src/app/js/vendors/` 无关，摘除主窗 vendor 不影响其运行；其退役须随采集窗 API 层迁移另行推进。
...
- **E6-9（`2d61c1a8`，P1-c-3）**：设置族（新增 `core/settings.ts`：preferences/theme/zoom/shortcut/scrollBehavior/broadcast，merge 逐字对齐 shims、落盘经全局 electronSettings 零改动）、`regenerate-palette`、库族、缩略图族、剪贴板导入族、导入族共 **21 个频道**迁入 routeDesktop（语义逐字对齐含失败分支）；`react-ipc-bridge-routing` 扩展断言 21 频道走接缝不泄漏回总线。
...
- **R4 切片④（台账闭合化：把「还剩多少」从印象变成可测的账）**：切片③的守卫只保护
  「已收敛域」，对剩余 229 个字段的判定是「已注册就算数据」——于是**函数挂载槽混在数据
  字段里**，既看不出真实待办量，也挡不住新引入的字符串函数挂载。本切片把守卫升级为
  **闭合分类台账**，三类互斥且必须覆盖每一处字符串键写入：
  - ①`CONVERGED`：已收敛域，字段必须走具名写点，再出现字符串键即失败。
  - ②`LEGACY_SCOPE_SLOTS`（57 个，**附归类理由**）：写的是函数/单例/命名空间对象/常量，
    属 **R6 退役面**而非 R4 数据字段——16 个旧面板派发槽、21 个标签管理器/文件夹落盘/
    通知/键位函数槽、13 个命名空间与常量挂载（`eagle`/`inspector`/`TagManager`/`options`/
    `folderIcons`/`fixUtils`/音效对象/`MAX_*` 等）。**清单不得变成僵尸账**：登记项若已无
    写入点即失败。
  - ③`PENDING_DATA_FIELDS`（172 个）：R4 待办数据字段。**未登记即失败**——新增的字符串键
    数据写入必须要么收敛、要么说明为何是挂载槽。
  - ④**函数值站点强制归类**：写入值是 `function` 字面量/箭头函数的站点必须落在
    `LEGACY_SCOPE_SLOTS`（实参提取做了括号与字符串感知，`(e.target as HTMLInputElement).value`
    这类合法数据写法不会误判）。这是 R4 「删除字符串函数挂载」的直接守卫。实测函数值站点
    24 处，全部在清单内。
  - **实测台账**：已收敛 27 字段/6 域；挂载槽 57；**R4 待办数据字段 172 个 / 464 处写入点**；
    注册表 315 字段。写入点最多：`isCropMode(8) lastZoomMode(7) selectedSmartFolders(7)
    boxContianerWidth(6) currentFolderChildren(6) folders(6) isMaximize(6) isRotating(6)
    lastIndex(6) selectedSmartFoldersMappings(6) zoomFitSize(6)`。
  - **反向验证**：临时探针文件同时植入三类违规（未分类新字段 / 已收敛字段回退 / 数据
    字段写入函数值），守卫逐一指名 FAIL；删除探针即 OK。
  - 含义：切片③的「未注册数据字段 = 0」仍是真命题，但**它不再被当作 R4 完成的证明**——
