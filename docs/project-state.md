...
> 最后更新：2026-09-16（**M0–M8 全部完成并收官**；验收报告 `docs/m8-acceptance-report-2026-09-16.md`）
> 当前套件 **102 项** + ARTIFACT **6 项** + EXTRA_REGRESSION **7 项** + 后端链 **47 项**；最近提交 `d4feb9f3`。
> **最终验收已完成**：四段全 PASS（`FRONTEND_ACCEPTANCE_ALL_GREEN`：static 5/5 / build / artifact 6/6 /
> regression 套件 102 项 ALL GREEN + 套件外 7 项全 PASS）；`backend` 段在 W56 修好 cpSync 后补跑通过
> （隔离全量回归 + `npm test` 47/47）。**未验证项与已知缺口见验收报告 §5 的 10 条**——
> 勿把「四段全绿」读成"没有边界"。
...
| M8 最终收官 | ✅ **已完成** | 验收报告见 `docs/m8-acceptance-report-2026-09-16.md`。四段矩阵 `FRONTEND_ACCEPTANCE_ALL_GREEN`（static 5/5 / build / artifact 6/6 / regression 套件 102 项 + 套件外 7 项全 PASS）；补跑的 `backend` 段通过（隔离全量回归 + `npm test` 47/47）。**M0–M8 全部收官**；未验证项与已知缺口见报告 §5 的 10 条 |
...
| **W49** | task_3f0dba9b0abe | ctx_ff09c149a94b | **M8-1 最小隔离部署副本：完整启动且不读取源工作区** | ✅ **已交付核验并集成（`0237ec17`）**，登记为 `ARTIFACT_TESTS` 第 6 项。我方独立复跑：`ISOLATED_DEPLOYMENT_OK`，**探针 violations 为空**（没有任何读取落在源工作区）。**它顺带查出发布内容本身两处漏拷 → 派 W51** |
| **W51** | task_8d087bcaa3d9 | ctx_d1335d7c0825 | **M8-3 补齐发布清单漏拷（`src/package.json` + 三个 `src/node_modules` 真实裸模块）** | ✅ **已交付核验并集成（`3191e6c8`）**。我方独立复跑：build 17 条登记资源、`publish-asset-manifest` 8/8 + 两条负向探针跑红、`dist-entry-check` FAIL 0/304；产物实测 `/src/package.json` 读回 `{version:4.0.0, buildVersion:20260401, buildNumber:23}`（原缺口正是这两者为空）；`isolated-deployment` 的 registeredGaps 只剩 1 条 |
| **W52** | task_32f3619c72de | ctx_8ab4a3dea385 | **M8-4 工作台缩略图缺图占位（依据 D33）** | ✅ **已交付核验并集成（`69b012ce` + 登记 `6361f0a7`）**。我方独立复跑：`WORKBENCH_THUMB_PLACEHOLDER_OK`（三组卡片 DOM 快照，两组占位盒与被替换图片盒同为 171×128）、typecheck 0 诊断、frontend-public-policy 9/9。**后端契约与 URL 一字未动** |
| **W53** | task_e2efc01862c9 | ctx_29244f85cae4 | **M8-R3 只读审计：重命名后缩略图失配根因（依据 D35）** | ✅ **已交付并核验**（`outputs/research-thumbnail-rename-2026-09-16.md`）。**结论推翻了原假设**：与改名无关，真因见 D36 |
| **W54** | task_153fd89ec750 | ctx_cdfc22ec974e | **M8-5 修 `/api/item/thumbnail` 恒 404 + 补两类门禁（依据 D36）** | ✅ **已交付核验并集成（`35a874fb`）**。修法为 `server.js:26` 加回一个标识符（1+/1-）。**我方独立做了两条负向自证**：去掉该 import ⇒ 类别门禁报 `server.js:2056:19 ensureThumbnail is called but is neither declared nor imported`、行为门禁报 `expected 200 … got 404`；恢复后双绿。`workbench-thumbnail-placeholder` 也在主工作区补跑通过（该测试不在 W54 的 worktree 里） |
| **W56** | task_3196a685a00e | ctx_7100c075f03a | **M8-7 后端 `fs.cpSync(recursive)` 改手动递归拷贝（依据 D40）** | ✅ **已交付核验并集成（`11820046` → `f82a05f2`）**。四处替换为共享模块 `backend/src/copy-tree.js`；**字面量 `npm test` 47/47 全部 exit=0**，那 11 个「因链断而从未执行过」的测试**首次跑起来、全部通过、未暴露新问题**。符号链接处理比 spec 建议更保守（显式重建链接而非 `copyFileSync`）。**我方定向复核**：`library-migration` 通过（该测试在本机从未成功过）、`backend-import-integrity` 48 文件/2834 调用点 0 悬空、`typecheck` 0 诊断、`item-thumbnail-endpoint` 通过 |
| **W55** | task_57d5c7d2a598 | ctx_37d54c6b2038 | **M8-6 固化部署根布局（依据 D34）** | ✅ **已交付核验并集成（`f7ebe2df`）**。新增 `docs/deployment-layout.md`；`isolated-deployment` 的 `RUNTIME_ROOT_GAPS` 由「只打印」升级为**会失败的断言**（三段布局 + 7 条耦合点解析 + src 与发布树 2054 文件逐一同构 + 三次变异均判红）。**我方另撤掉最后一条已失效豁免并加了反向校验（`58fa2ca5`）** |
| **W50** | task_cf5b97d85916 | ctx_7d776f5ebd38 | **M8-2 修视频夹具：MediaRecorder 的 WebM 补真实 Duration** | ✅ **已交付核验并集成（`2b475e81` + 登记 `76221352`）**。我方独立复跑：新单测 8/8、负向自证 6/6 跑红、**`video-detail-mode-closed-loop` EXIT=0 且首次拿到原生分支（readyState=4、duration=1.2365）**、四个夹具消费方全 OK、typecheck 0 诊断。**这个既有红已归零** |
...
| D14 | **M2-2 的能力取舍裁决：`http`/`https` 与 `JsonRestServer` 保持显式失败；`archiver`/`fast-glob` 维持 no-op** | 三者均经消费者取证：`http`/`https` 唯一消费者 `urlEnlarger.#checkURLByEagle` 被 `isRunningInEagleApp` 门控且**无 try/catch**（贸然给真实实现会重新引入「抛点落在无保护表达式上」）；`JsonRestServer` 真实实现 `src/my_modules/json-rest-light` 首行即 `require('http')`，browser-connected 态确无该能力（41595 的 API 面由后端承担，见 `installBrowserFetchRewrite`）；`archiver` 仅见于**从不被加载**的 `src/app/js/plugin/index.js` 与渲染层 0 消费者的 `src/my_modules/zip-folder`，`fast-glob` 唯一消费者在独立后端进程。两者已登记进 `capabilities.gaps`，不是静默成功。**~~待证项：Electron 下 `nativeRequire('http')` 可能可用~~ → 已于 2026-09-17 取证，见下方「D14 取证结论」** |
...
| **D31** | **M8-1 的方法论：探针要判「实际读了什么」，不要判「grep 不到路径」** | 隔离部署门禁把「不读取源工作区」做成**文件访问探针**（拦截 fs 读取类方法并记录命中集合），而不是静态扫描。正因如此它同时给出了两个方向的结论：① 探针 violations 为空 ⇒ **确实没读源工作区**；② 但发布内容本身漏拷了两项（见 D32）。**若只做静态 grep，第 ① 条会被当成"没问题"，第 ② 条则根本不会出现。** |
| **D40** | **`fs.cpSync(recursive)` 在本机打死进程——用户裁决「改那三处」** | M8 最终验收的 `backend` 段失败，Coordinator 定位到真因：`fs.cpSync(src, dst, {recursive:true})` 在本机**源路径含任意非 ASCII 字符**时就把进程打死（退出码 `3221226505` = `0xC0000409` STATUS_STACK_BUFFER_OVERRUN，**无异常栈、catch 不到**）。**根因由 W56 经受控二分更正**（我先前「嵌套子目录树触发」是实验假象，已更正）：与目标路径、树形状/深度/宽度/体积无关，非 ASCII 的种类不限中文（重音/西里尔/emoji 同样触发）。**我方独立复核的两格对照**：ASCII src → ASCII dst `exit=0`；ASCII src → 非 ASCII dst `exit=0`；**非 ASCII src → ASCII dst `exit=0xC0000409`**。本仓工作区路径含中文（`... - 副本`），真实用户的库路径也常含中文 ⇒ **产品级缺陷，不是夹具特有**。**后果两层**：① `POST /api/library/migrate`、`exportLibrary`、备份恢复三个端点在本机不可用；② `npm test` 是 `&&` 链，在第 **36/47** 步（`library-migration`）断掉，**后面 11 个测试在本机从未执行过**（`security` / `roadmap-panels` / `library-transaction-faults` / `library-crash-recovery` / `library-concurrency` / `library-consistency-scan` / `library-backup-restore` / `library-repair-plan` / `source-mode-api` / `source-mode-thumbnails` / `source-mode-watch`）。**用户裁决改那三处**（另加 `tests/roadmap-panels.mjs:139` 那处，它在链的第 37 步）。依据是仓内**已用过两次**的同一手法：`backend/src/server.js:2070-2083` 的 `copyInfoDirRecursive()`（注释明写「宿主 fs.cpSync(recursive) 缺陷绕行」）与 `frontend/vite.preview.config.mjs` 的 `copyTree()`。**W56 / M8-7** 执行，**结果：那 11 个测试首次全部通过、未暴露新问题；`npm test` 47/47 全 exit=0。** |
| **D38** | **豁免表要有反向校验：登记了却没命中 ⇒ 判红** | W54 修好那个 404 后，隔离部署门禁报 `GAPS_REGISTERED 0/1`（最后一条豁免已不再命中），而门禁对"登记了却没命中"是静默容忍的——这正是「绿跑把已修好的缺陷悄悄藏住」的风险面。**处置**：删掉该条目（表变空，并在注释里写清"空表是当前正确状态，不是待办"与新增条目的门槛），并加 `assert.deepEqual(staleGaps, [])` 做反向校验（同 D4 口径「登记项若已存在会 FAIL，防僵化」）。此后该路径再 404 直接判红 |
| **D36** | **`/api/item/thumbnail` 自 2026-08-04 起对任何条目恒 404——悬空标识符被自己的 catch 吞掉** | M8-R3 审计推翻了我原先「改名后缩略图失配」的假设：真因是 `backend/src/server.js:2056` 调用了 `ensureThumbnail(...)`，而 **`:26` 的 import 里根本没有它**（只有 `thumbnailPath`）⇒ 运行期 `ReferenceError`，被**紧邻的 `catch (err) { thumbPath = ''; }` 静默吞成 404`**。**引入回归的提交是 `8cf92c55`（2026-08-04「实现统一缩略图任务服务」）**：它把 `import { ensureThumbnail, generateThumbnail, generateThumbnailAsync, thumbnailPath }` 收窄成只剩 `thumbnailPath`（其余调用点迁到新服务），**漏改了 `:2056` 这一处**。后果：该端点对**所有条目、所有扩展名**恒 404，唯一消费者是工作台卡片图。Coordinator 另做了一遍 `backend/src` 悬空调用粗筛，逐个核对后确认**除这一处外无同类**（其余命中都是同文件内的本地定义）。**处置**：加回一个标识符 + 补行为门禁 + 补一类「调用但未导入」的静态门禁（W54 / M8-5，含诚实降级条款）。这也印证了 W52 的取舍——`onError` 兜底是必需的，`noThumbnail` 单条覆盖不到 |
| **D39** | **`backend/src` 首次有了「调用但未导入」的类别门禁** | M8-5 新增 `tests/backend-import-integrity.mjs`：用 TypeScript 编译器建作用域链，报出「被调用却既无本地定义也无导入」的标识符。**它精确命中过 `server.js:2056:19`**（我方独立复跑验证：去掉 import 即报红，恢复即绿）。worker 的评估结论是**不降级**——47 文件 / 2828 调用点 0 假阳性，对 25+ 易误伤构造（解构/默认参/rest/catch/for-of/switch 块/类表达式/命名函数表达式/可选调用/标签模板）同样 0 假阳性；另加 500 调用点空转护栏防它静默退化成 no-op。**这一类缺陷（改 import 时漏改调用点）此前没有任何门禁覆盖**，`8cf92c55` 的那次回归正是从这漏过去的 |
| **D37** | **`skipDangerousModePermissionPrompt` 反复被抹：改为「派单走脚本」而不是「派前记得查」** | D29 记了"每次 worker-start 前都要查"，但同一天内它又复发（派 W54 时 `--list` 刚查过是 `true`，几分钟后就变 `undefined`）。**处置：新增 `outputs/_start-worker.mjs`**——它先确保标志在位（node 读改写回，不用 Python），再启动 worker，并把 dispatch id / stage / 退出码打出来。此后一律走它派单，不再依赖手工检查 |
| **D33** | **用户裁决：`/api/item/thumbnail` 保持 404 契约，改在前端补占位** | 三选一（保 404 + 前端补占位 / 后端改 200+占位 / 只登记），用户取推荐项。理由：改状态码会动既有 API 契约、风险大于收益；破图是纯呈现问题。落地 = `Workbench.tsx:846` 的 `<img>` 补 `onError` 兜底（并优先复用仓内既有的 `item.noThumbnail` 约定，但**不能只靠它**——本次 404 的那条恰恰带不到该标记）。**后端一行不改**（W52 / M8-4） |
| **D34** | **用户裁决：固化「部署根布局」，零产品代码改动** | 三选一（固化布局 / 改代码收敛到单一可配根 / 只登记），用户取推荐项。**关键事实**：`projectRoot = path.resolve(<backend/src>, '../..')`（`backend/src/server.js:76`、`library-store.js:8` 同款），所以只要部署根是 `{backend/, electron/, src/（已发布的 src 树）}`，那四处 `projectRoot/src/...` 自然解析到**发布树**而非源工作区——W49 的隔离门禁就是这么构造的，实测探针 violations 为空。落地 = 把这条布局写进文档 + 让 `isolated-deployment` 门禁显式**断言**该布局（**等 W51 落地后再做，避免同文件并发改**） |
| **D35** | **用户裁决：对「重命名后缩略图失配」先派只读审计定位，不直接修** | 三选一（派只读审计 / 只登记 / 直接派修复批），用户取推荐项。**已掌握的静态事实**：`thumbnailPath = <lib>/images/<id>.info/<item.name>_thumbnail.png` 用的是**当前名字**（`backend/src/thumbnailer.js:7-9`），`ensureThumbnail` 落空即回退重新生成（`:92-96`），文本类重生不出 ⇒ 404。**疑似根因**：重命名后磁盘上的缩略图文件仍是旧名。审计要查清写侧/改名侧/读侧三张表、`/file/` 为何能取到、影响面、是否逐字移植、有无测试覆盖（W53 / M8-R3） |
| **D32** | **发现两处「动态资产漏拷」——正是任务书 M7 点名的那一类** | M8-1 在**仓库自己的 `dist/frontend`** 上复现（与隔离无关）：① `src/package.json` 404，而运行期有 8+ 处 `req(appRoot.path + '/package.json')`（panelState / bundleGlobals / apiServerDomain / shell.tsx / DetailViewer / Inspector / PluginCenter）⇒ 产物里 `version`/`buildVersion` 取不到（旁证：实机日志里出现过 `app-notifications?...&version=&buildVersion=` 两者皆空）；② `src/node_modules/{compare-versions,stopword,electron-referer}` 未登记，而 `moduleRegistry.ts:545/546/550` 把三者登记为**真实裸模块**（M2-2 修回归时补的）⇒ 产物里落 `genericStub`。**处置：只改发布清单与门禁、不改产品代码**（W51 / M8-3），并把 `isolated-deployment` 的 `KNOWN_DEPLOYMENT_GAPS` 相应删条——删掉后该路径再 404 即判红 |
| **D30** | **既有红 `video-detail-mode-closed-loop` 的定性：改夹具，不改产品** | M8 首轮 L3 抓到的唯一套件外红。二分到会话前的 `ca57829f` 复现同形失败 ⇒ 非本轮回归；探针实测唯一不成立的是 `duration === Infinity`（`w=160 h=90 readyState=4`，视频完全可播放）；判据 `detailHooks.ts:357` 是 bundle 64843-65684 的**逐字移植**，按硬约束 3 不得改。根因是 MediaRecorder 产出的 WebM 不含 `Info.Duration`。**处置：给夹具做 EBML Duration 注入**（W50 / `2b475e81`），产品一行未动 |
| **D29** | **`skipDangerousModePermissionPrompt` 在**同一次会话内**也会被静默抹掉——派单前每次都要查** | 2026-09-16：会话早些时候我核对过该字段为 `true`（W44–W48 五个 worker 全部正常启动）；数小时后派 M8-1/M8-2 时**两个 worker 同时**卡在 Bypass 确认框（`turn_start_unexpected`、终端停在 `BPS ...>`）。复查发现字段**又变回 undefined**。⇒ 「开始时查过一次」不成立，**每次 `worker-start` 之前都要查**。处置仍是 §7.1 那套：node 读改写回（不用 Python）+ `worker-stop` + `--retry-of` + `path:<绝对路径>`。已同步进记忆 `env-orca-worker-bypass-prompt.md` |
...
| **D41** | **D14 待证项已取证：Electron 渲染层 `require('http')` 实测可用；但裁决维持「显式失败」，理由升级为更强的一条** | 取证脚本 `outputs/d14-capability-probe.cjs`（**一次性取证，不是门禁**），实测 Electron 22.3.7 / Node 16.17.1，窗口 `webPreferences` 与生产一致（`nodeIntegration:true` + `contextIsolation:false` + `sandbox:false`）：渲染层 `http`/`https`/`net`/`node:os`/`node:fs`/`path`/`child_process`/`electron` **全部 ok**（`http` 取到 `METHODS`/`STATUS_CODES`/`Agent`）⇒ 「`nativeRequire('http')` 可能可用」从**推测变为结论**。**但裁决不变**（`http`/`https`/`JsonRestServer` 仍保持显式失败），且理由更强：不是「能力不存在」，而是**唯一消费者 `urlEnlarger.#checkURLByEagle` 无 try/catch 且在 `isRunningInEagleApp` 门控下**，贸然接线会把「抛点落在无保护表达式上」重新引入（D14 原判据成立，只是不再依赖推测）。**若将来要接线，前置条件是先给该消费者加 try/catch 或把调用点整体纳入能力门控**，而不是直接把 `nativeRequire('http')` 接上去。**同批顺带闭合 R1-2**：主进程 `app.getPath('userData')` 返回**真实路径** `C:\Users\Administrator\AppData\Roaming\Electron`（直接 `npx electron` 跑、未设 app name，故名为 `Electron`；真实运行按 app name 走，取证看的是「是否真值」而非具体值）；渲染层 `ipcRenderer.sendSync('app:get-path','userData')` 取到的值与主进程**完全一致**，`temp` 同理，空名/非法名返回 `''`（按设计不伪造）。⇒ `/mock-user-data` 那条路的新增真值通道已证实可用 |
| **D42** | **本机 Electron 类测试失败的头号原因是 GPU 进程 FATAL，不是沙箱；`--in-process-gpu` 可解，已做成默认关闭的环境变量逃生口** | 此前（含验收报告 §1.3、D22 的「L3 必须普通终端跑」）把 Electron 类测试的红归给 WorkBuddy 沙箱。**实测推翻**：`node tests/document-viewer-ui-closed-loop.mjs` 不加任何改动即失败，日志是 `gpu_process_host.cc(991) GPU process exited unexpectedly`（连发 9 次）→ `gpu_data_manager_impl_private.cc(440) GPU process isn't usable. Goodbye.`，Chromium 直接杀掉浏览器进程，表现为「无故超时」。**开关对照**（Electron 22.3.7）：`--in-process-gpu` ✅、`--disable-gpu-sandbox --disable-gpu` ✅、**单独 `--disable-gpu` ❌**、`--use-gl=swiftshader` ❌（更糟）。**处置**：在 `electron/main.cjs` 最顶部加**默认关闭**的逃生口——仅当 `EAGLE_ELECTRON_IN_PROCESS_GPU=1` 时 `app.commandLine.appendSwitch('in-process-gpu')`（实测与命令行传参等价：同一测试由 EXIT=1+GPU FATAL 变为 `DOCUMENT_VIEWER_UI_CLOSED_LOOP_OK`）。**为什么不动 18 个直接 spawn Electron 的测试文件**：那是把同一处环境适配抄 18 遍；逃生口只需一处，且生产默认完全不变。**代价必须写明**：它改变的是渲染后端，故任何用它跑出的结论都必须在归档里注明（见工作记录 §15），不得与「生产配置下的结论」混为一谈 |
