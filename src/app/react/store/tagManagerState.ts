...
export function unbindTagManagerSync(): void {
  if (!bound) return;
  bound = false;
...
export function bindTagManagerSync(): void {
  ...
  const unsub1 = useBodyState.subscribe(() => syncTagManagerFromScope());
  const unsub2 = useListState.subscribe(() => syncTagManagerFromScope());
  unbindSubscriptions = () => {
    unsub1();
    unsub2();
  };
