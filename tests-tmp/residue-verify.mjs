...
// b1-9ay 修复验证（sweep B1-B7 定论的三个真问题的回归）：
...
let failure = null;
const failures = [];
const watchdog = setTimeout(() => { console.log('RESIDUE_VERIFY_WATCHDOG'); process.exit(2); }, 480000);
...
async function main() {
  ...
  console.log(`RESIDUE_VERIFY ${failures.length === 0 ? 'OK' : 'FAILED'} ${JSON.stringify({ failures })}`);
}

try {
  await main();
} catch (err) {
  console.error(`RESIDUE_VERIFY_FAIL ${err.message}`);
