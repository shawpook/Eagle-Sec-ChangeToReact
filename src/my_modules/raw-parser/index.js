...
function extractRawThumbnail (filePath, buf, output, callback) {
	try {
		filePath = path.normalize(filePath);
		output = path.normalize(output);
		let jpegBuffer, img;

		console.time("generate with nativeImage");
		ipcRenderer.invoke('nativeImage.createThumbnailFromPath', {
        	...
        	try {
	        	jpegBuffer = fs.readFileSync(output);
	        	img = nativeImage.createFromBuffer(jpegBuffer);
	        }
	        catch (err) {}
	        console.timeEnd("generate with nativeImage");

			if (!jpegBuffer || img.getSize().width === 0 || img.getSize().width === img.getSize().height) {
				try {
					console.time("generate with dcraw");
					jpegBuffer = dcraw(buf, { extractThumbnail: true });
					img = nativeImage.createFromBuffer(jpegBuffer);
					console.timeEnd("generate with dcraw");
					if (!jpegBuffer || img.getSize().width === 0) {
				        callback({});
						return;
					}
				}
