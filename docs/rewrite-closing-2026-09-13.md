...
| P1 通道桥 | 🟡 **P1-a（`dfd95ab8`）+ P1-b（`df043f08`）+ P1-c-1（本批）已落地**，P1-c-2…e 未落地。P1-a：`core/channelBridge.getIpcBus()` 收口主窗 17 文件 24 处取总线表达式。P1-b：`preload.cjs` 新增通用 `ipc` 桥；`getIpcBus()` 升级为**接缝 facade**——事件面前转 shims 总线，发送面按通道二分（纯原生直通 6 频道 + `smoke:*` 走 preload，其余走 shims 路由表）。P1-c-1：再迁 5 个**无副作用**的 desktopApi 频道入 facade（`folders-change` / `open-preview-window` / `export-images` / `export-as-folder` / `cancel.all`，含 `preview:operation-result` 等合成事件），语义逐字对齐 shims。`eagleGlobals.ipcRenderer()` 返 facade（不得直返 `desktop.ipc`）。`tests/react-ipc-bridge-routing.mjs`（已入套件）用注入式假 `eagleDesktop` 断言单路由不变量：原生/desktop/ shims 三面各归其位、无泄漏无双重路由、事件前转、合成事件。**未落地**：P1-c-2 起是 **item 持久化路径**（`images-change`→`updateMany`，含 `mergeCachedItems`/`emitImportedItems`/`itemUpdateQueue` 等 helper）与导入/剪贴板族 —— 属应用最关键的写路径，且现有测试只能在 Electron 下验（浏览器态仍由 shims 承接），验证可靠性依赖 `main-ui-workflow`（当日实测为其既有负载 flake 所扰）。逐信道地图见 `docs/e5-5-ipc-inventory-2026-09-14.md` |
...
| P3 source-mode UI | 🟡 部分。**实测更正**：该块在 React 下可开/关（`Sidebar.tsx:699` 有意保留 `ng-click` 兼容钩子供 `installModeSwitch` 识别）+ 可「添加来源文件夹」，但 `renderSourceModeSidebar` 渲染出的 `data-source-*`（点击目录行 / `data-source-action="toggle-root"` / `data-source-rescan` / `data-source-remove`）**全仓无任何监听器**（shims 与 React 皆无），而唯一意图实现这些交互的 `handleSourceSelectFolder` / `handleRescanRoot` / `handleRemoveRoot` / `openSourceFolderInAngular` **零调用点**。故 P3 不是搬迁，而是**功能补建**（浏览来源素材 / 重扫 / 移除的交互语义需先定）。本批已安全清掉这 4 个不可达函数（-83 行，`source-mode-ui` 复跑绿）；块本体迁移至此搁置 |
| P4 浏览器 mock 隔离 | ⬜ 未落地 |
