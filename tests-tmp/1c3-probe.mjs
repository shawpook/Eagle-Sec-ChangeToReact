...
try {
  ...
  await assertExpr('c3-contract', `(() => {
    ...
    const missing = spot.filter(k => typeof c[k] !== 'function');
    if (Object.keys(c).length !== 230 || missing.length) { window.__diag = { count: Object.keys(c).length, missing }; }
    return Object.keys(c).length === 230 && spot.every(k => typeof c[k] === 'function');
  })()`);

  try {
    const d = await page.send('Runtime.evaluate', { expression: 'JSON.stringify(window.__diag || null)', returnByValue: true });
    console.log('C3_DIAG ' + d.result.value);
  } catch (err) {}
