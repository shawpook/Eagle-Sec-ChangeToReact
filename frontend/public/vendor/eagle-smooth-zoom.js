 ...
 *
 * 与原文件的唯一差异（5 + 1 处，机械替换，其余逐字节相同）：应用层胶水的 Angular 依赖
...
if (!self._mousedown) return;
			...
			if (e.type == 'mousemove') {
				// 計算新位置
				const newX = e.pageX - self.$holder.offset().left - self.offX;
				const newY = e.pageY - self.$holder.offset().top - self.offY;
				
				// 檢查位置是否真的改變（避免微小抖動觸發渲染）
				const threshold = 1; // 像素閾值
				if (self._lastDragX !== undefined && self._lastDragY !== undefined) {
					const deltaX = Math.abs(newX - self._lastDragX);
					const deltaY = Math.abs(newY - self._lastDragY);
				...
				// 記錄位置
				self._lastDragX = newX;
				self._lastDragY = newY;
				
				self.setDraggedPos(newX, newY, self._sc);
				self._recent = 'drag';
				self._dragging = true;
		...
		***********************************************************************************************************************/
		mouseUp: function (e) {
			...
			self.pointers = [];
			// 重置拖曳追蹤變數
			self._lastDragX = undefined;
			self._lastDragY = undefined;
		...
		mouseWheel: function (e, delta) {
			...
			if (delta > 0) {
				if (self.rA != self.zoom_max) {
					...
					self._wheel = true;
					self.cFlag._zi = true;
					// cancelAnimationFrame(self.ani_timer);
					self._playing = true;
					self.Animate();
					self.cFlag._zi = false;
				}
		...
		***********************************************************************************************************************/
		Animate: function (noAnimate) {
			...
			if (self.ani_end && !self._dragging && self._recent != "drag") {
				...
				// cancelAnimationFrame(self.ani_timer);
				// self.ani_timer = requestAnimationFrame(function () {
				// 	self.Animate();
				// });
			}
		},
		...
		***********************************************************************************************************************/
		haltAnimation: function () {
			// cancelAnimationFrame(this.ani_timer);
			this._playing = false;
			this._recent = '';
		},
